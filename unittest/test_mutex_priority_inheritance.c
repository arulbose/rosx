...
void task_3(void)
{
	while(1) {
		...
		rx_mutex_lock(&m, OS_WAIT_FOREVER);
                rx_early_printk("Mutex taken by task 3 \n");
		rx_mutex_unlock(&m);
                rx_early_printk("Mutex unlock task 3 \n");
...
void task_2(void)
{
	rx_early_printk("entering task_2\n");

	while(1) {
#if 1
		if(OS_OK != rx_mutex_lock(&m, OS_WAIT_FOREVER)) {
			rx_early_printk("Failed to get Mutex task 2 \n");
			rx_suspend_task(MYSELF);
		} else {
			rx_early_printk("got the mutex task 2\n");

		}
                rx_ssleep(9);
		rx_mutex_unlock(&m);
#endif
                rx_early_printk("Mutex unlock task 2 \n");
		rx_suspend_task(MYSELF);
	}
}
...
void task_1(void)
{
	rx_early_printk("entering task_1\n");
        rx_ssleep(7);
	rx_early_printk("task_1 coming out of sleep\n");
	while(1) 
	{
